...
'''All processed information is saved in a .json'''
def writer_to_json(jobs_list, config):
    ...
    for job in jobs_list:
        ...
        jobs_json.append({'Company' : jobs.company_fix, 'Position': jobs.position, 'City': jobs.city, \
                          'Salary': jobs.salary, 'Interest' : int(jobs.intrest), 'Acept': int(jobs.acept)})
...
'''All processed information is saved in a .csv'''
def writer_to_csv(list_of_dict, choice, config):
    try:
        ...
        config.main_logger.info(f'All information is saved in a CSV file')
        config.console_logger.info(f'All information is saved in a CSV file')
...
'''All processed information is saved in Excel file'''
def write_to_xlsx(list_of_dict, choice, config):
    try:
        ...
        for job in list_of_dict:
            ...
            row += 1
        wb.close()
        config.main_logger.info(f'All information is saved in an Excel file')
        config.console_logger.info(f'All information is saved in an Excel file')
    except FileNotFoundError:
        config.console_logger.error('Something is wrong with the document or directory')
        config.main_logger.error('Writer_to_csv give FileNotFoundError')
